...
export const addReview = async (req, res) => {
  try {
    ...
    if (order.status === 'COMPLETED') {
      ...
      await createNotification(order.seller, `${req.session.user.firstName} ${req.session.user.lastName} wrote you a review for order #${order._id}`, '/account/profile#reviews');
...
export const updateReview = async (req, res) => {
  try {
    ...
    const average = arr.reduce((p, c) => p + c, 0) / arr.length;
    await User.findByIdAndUpdate({ _id: order.seller }, { rating: average });
    await createNotification(order.seller, `${req.session.user.firstName} ${req.session.user.lastName} edit his/her review for order #${order._id}`, '/account/profile#reviews');
    return res.status(200).json({
      success: 200,
